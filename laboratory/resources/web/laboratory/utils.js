        ...
        getAssayDetails: function(config){
            ...
            multi.add(LABKEY.Assay.getById, {
                ...
                success: function(results){
                    ...
                    var keyText = (results[0].domains ? Ext4.encode("['" + Ext4.Object.getKeys(results[0].domains).join("','") + "']") : 'none');
                    LDK.Assert.assertNotEmpty('LABKEY.Assay.getById did not return information for the Batch domain for [' + results[0].name + '].  Keys were: ' + keyText, ret.domains.Batch);
                    LDK.Assert.assertNotEmpty('LABKEY.Assay.getById did not return information for the Run domain for [' + results[0].name + '].  Keys were: ' + keyText, ret.domains.Run);
                    LDK.Assert.assertNotEmpty('LABKEY.Assay.getById did not return information for the Results domain for [' + results[0].name + '].  Keys were: ' + keyText, ret.domains.Results);
